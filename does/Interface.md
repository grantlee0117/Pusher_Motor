...
**功能**：触发一次推料动作。 
**行为**：设置 `pusher_motor_start_flag = 1`，状态机将在下一次 `pusher_motor_loop()` 中从 IDLE 进入 WAIT 状态。 
**调用场景**：
...
| 参数 | 类型 | 范围 | 默认值 | 说明 |
|------|------|------|--------|------|
| `direction_time_ms` | `uint32_t` | 1 ~ 60000 | 1500 | 单次运行时间（ms） |
| `pwm_duty` | `uint32_t` | 0 ~ 500 | 30 | PWM 占空比（0=全速，500=停止） |
...
| `USART1_TX_Pin` | PA9 | 调试串口发送（TX） |
| `USART1_RX_Pin` | PA10 | 调试串口接收（RX） |
