...
本项目使用 `arm-none-eabi-gcc` 交叉编译工具链，并通过 CMake preset 调用 Ninja 生成构建目录。

需要的工具：

- CMake 3.22+
- Ninja
- Arm GNU Toolchain，即 `arm-none-eabi-gcc`
...
如果 `arm-none-eabi-gcc` 不在 `PATH` 中，CMake 配置阶段会失败。当前工具链文件是 `cmake/gcc-arm-none-eabi.cmake`，它默认查找 `arm-none-eabi-gcc`、`arm-none-eabi-g++`、`arm-none-eabi-objcopy` 等命令。

## 编译流程

推荐直接使用 CMake preset。Debug 构建流程如下：
...
根目录 `Makefile` 和 `Scripts/build.sh` 只是对 CMake preset 的便捷封装，适合日常使用。
