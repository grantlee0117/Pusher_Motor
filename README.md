...
├── docs/                        # 当前文档索引、构建教程、开发札记、串口命令和过期历史文档
...
- `docs/` 放当前仍可直接使用的说明文档，`docs/README.md` 是文档总索引；接口、串口命令、硬件资料发生变化时，需要同步更新相关文档。
- `docs/dev-notes/` 用于记录开发心得、踩坑和阶段性判断；成熟内容应再沉淀到根目录 README、`docs/Build.md`、`docs/串口调试命令.md` 或对应目录 README。
- `docs/deprecated/` 只保存已经过时但仍有参考价值的历史文档，不作为当前开发事实来源。
...
- [docs/dev-notes/README.md](docs/dev-notes/README.md): 开发札记目录说明
...
文档分层原则：根目录 README 只保留团队入口、目录边界和关键协作约定；`docs/` 保存可直接用于当前开发的说明；开发过程中的临时经验先写入 `docs/dev-notes/`，稳定后再整理到正式文档。
