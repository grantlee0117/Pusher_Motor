...
> 图中展示了主控板、电机驱动板、电机模块之间的实际接线。注意：图片标注的"推料时间暂定3S"为早期设定，实际固件默认值已更新为 250ms，可通过 CLI 命令 `set direction_time` 调整。
...
#### 关键参数
```c
#define DEFAULT_DIRECTION_TIME_MS   250   // 默认运行时间（毫秒）
#define DEFAULT_PWM_DUTY            150   // 默认PWM占空比（0-500）
#define DEFAULT_WAIT_TIME_MS        250   // 默认等待时间（毫秒）
#define DEFAULT_ACCEL_TIME_MS       0     // 默认加速时间（毫秒，0表示不启用斜坡）
...
3. **运行阶段**：启动 PWM 输出；如果 `accel_time_ms` 大于 0，则从停止占空比 500 平滑过渡到目标占空比
...
| `set new_pwm_duty <value>` | `SERVICE` 模式下直接设置PWM占空比（不保存Flash） |
| `get direction_time` | 获取当前运行时间 |
| `get pwm_duty` | 获取当前 PWM 占空比 |
| `get wait_time` | 获取当前等待时间 |
| `get accel_time` | 获取当前加速时间 |
...
### 3. 系统模式模块 (system_mode.c)

#### 功能
- 显式定义系统身份和命令边界
- 防止运行中保存参数或重复启动
- 将售后手动 PWM 调试限定在 `SERVICE` 模式
...
| `BOOT` | 系统初始化中 |
| `IDLE` | 空闲，允许配置和启动 |
| `RUNNING` | 正在执行一次动作，禁止保存型配置和重复启动 |
| `SERVICE` | 售后调试模式，允许临时 PWM |
| `ERROR` | 错误保护预留模式 |

### 4. 参数管理模块 (params_manager.c)
...
typedef struct {
    ...
    uint32_t accel_time_ms;        // 加速时间（毫秒，0表示不启用斜坡）
...
```c
#define DEFAULT_DIRECTION_TIME_MS   250   // 修改默认值
#define DEFAULT_PWM_DUTY            150   // 修改默认值
