...
void pusher_motor_init(void)
{
    ...
    HAL_GPIO_WritePin(MOTOR_E2_DIR_PORT, MOTOR_E2_DIR_PIN,
...
/**
 * @brief 一键设置参数并启动电机
 * @param direction_time_ms 运行时间（毫秒），由 CLI 输入 a 乘以 100 后传入
 * @param wait_time_ms 等待时间（毫秒），由 CLI 输入 b 乘以 100 后传入
...
void pusher_motor_loop(void)
{
    ...
    case MOTOR_STATE_WAIT:
        ...
        if ((current_time - motor_start_time) >= params_manager_get_wait_time())
        {
            ...
            {
                // 无加速：直接输出目标占空比
                E1_Set_Duty(params_manager_get_pwm_duty());
                E2_Set_Duty(params_manager_get_pwm_duty());
            }
            else
            {
                // 加速模式：从停止状态（0）开始
        ...
        else if (acc_speed_value != 0)
        {
            ...
            if (accel_current_duty < target_duty)
            {
                ...
                if ((current_time - last_accel_tick) >= ACCEL_INTERVAL_MS)
                {
                    ...
                    E1_Set_Duty(accel_current_duty);
                    E2_Set_Duty(accel_current_duty);
                }
            }
        }
        break;
