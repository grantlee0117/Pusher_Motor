...
#define DEFAULT_ACCELERATION        0     // 默认加速度（0=无加速，1-50=加速步长）
```
...
3. **运行阶段**：
   - **无加速模式**（`acceleration = 0`）：直接输出目标占空比，运行 `direction_time_ms`
   - **加速模式**（`acceleration > 0`）：从占空比 500（停止）开始，每隔 10ms 逐步减少占空比（步长 = `acceleration`），直到达到目标占空比，然后继续运行直到 `direction_time_ms` 结束
...
| `set acceleration <0-50>` | 设置加速度步长（0=无加速） |
