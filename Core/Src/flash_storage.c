...
#define DEFAULT_ACCEL_TIME_MS 0       // 默认加速时间，0表示不启用斜坡
...
{
    return data->direction_time_ms + data->pwm_duty + data->wait_time_ms + data->max_speed_rpm + data->motor_mp_a_dir + data->motor_mp_b_dir + data->accel_time_ms;
}

/**
 * @brief 计算旧版参数结构校验和（旧固件没有加速时间字段）
 * @param data 参数结构体指针
 * @return 校验和
 */
...
uint32_t FlashStorage_Init(void)
{
    ...
    // 数据无效或读取失败，保持擦除态；由参数管理器使用RAM默认值。
    ...
    storage.accel_time_ms = DEFAULT_ACCEL_TIME_MS;
...
uint32_t FlashStorage_Read(FlashStorage_t *data)
{
    ...
    data->accel_time_ms = *(__IO uint32_t *)(FLASH_STORAGE_ADDR + 24);
...
uint32_t FlashStorage_Write(FlashStorage_t *data)
{
    ...
    status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, FLASH_STORAGE_ADDR + 24, data->accel_time_ms);
    if (status != HAL_OK)
    {
        HAL_FLASH_Lock();
        return status;
...
uint32_t FlashStorage_IsValid(FlashStorage_t *data)
{
    ...
    // 兼容旧版Flash布局：旧版 +24 位置保存的是校验和，+28 仍为擦除态
    if (data->checksum == 0xFFFFFFFF && data->accel_time_ms == CalculateLegacyChecksum(data))
    {
        data->accel_time_ms = DEFAULT_ACCEL_TIME_MS;
        data->checksum = CalculateChecksum(data);
        return 1;
    }

    if (data->accel_time_ms == 0xFFFFFFFF || data->accel_time_ms > 10000)
    {
        return 0; // 加速时间超出范围
    }
